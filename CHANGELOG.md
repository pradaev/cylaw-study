...
## 2026-02-09 (Vectorize re-embedding overhaul)

### Added
- **Contextual headers** — each chunk prepended with `Δικαστήριο: ... | Δικαιοδοσία: ... | Έτος: ... | Title`
- **Jurisdiction extraction** — parsed from ΔΙΚΑΙΟΔΟΣΙΑ field with path-based fallback (59.4% coverage)
- **Text cleaning** — strip markdown links, `*`/`#` formatting, C1 control chars, normalize unicode
- **ΑΝΑΦΟΡΕΣ stripping** — cross-reference sections removed before embedding
- **Tail chunk merging** — last chunks < 500 chars merged with previous
- **New Vectorize index**: `cyprus-law-cases-search-revised` (~2.07M vectors, metadata: +jurisdiction)
- **Separate pipeline commands**: `create-index`, `download`, `upload` (split from old `collect`)
- **Download retry logic** — exponential backoff, reduced parallelism (3 workers) to avoid 504s
- **Index comparison tool** (`scripts/compare_indexes.mjs`) — side-by-side old vs new index
- **Deep-dive diagnostic** (`scripts/deep_dive_query.mjs`) — full pipeline trace for a single query
- **Comprehensive chunker test** (`tests/test_chunker_all_courts.py`) — covers all 15 court types

### Changed
- **areiospagos reclassified** → `court_level=foreign` (was `supreme`). Updated in chunker, llm-client, retriever.
- **court_level filter** now supports `"foreign"` in addition to `"supreme"` and `"appeal"`
- **Vectorize index** switched to `cyprus-law-cases-search-revised` in all frontend/config files

## 2026-02-07 (late evening — Vectorize ingestion)

### Added
- **`scripts/batch_ingest.py`** — OpenAI Batch API pipeline for embedding all 149,886 documents into Cloudflare Vectorize. Pipeline: create-index → prepare → submit → status → download → upload. 50% cheaper than synchronous API, separate rate limits, no TPM throttling.
...
### Changed
- **Vectorize index `cyprus-law-cases-search`** marked as PRODUCTION — ~2.27M vectors from all 15 courts, OpenAI `text-embedding-3-small` (1536 dims, cosine). Later superseded by `cyprus-law-cases-search-revised`.
