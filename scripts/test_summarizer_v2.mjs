/**
 * Test: run the UPDATED summarizer prompt against E.R v. P.R
 * with the "foreign law" query.
 */
...
// Current prompt from llm-client.ts
function buildSummarizerPrompt(focus, userQuery, docId) {
...
async function main() {
  ...
  console.log(`Doc: ${DOC_ID} (${extracted.length} chars)\n`);
  ...
  const response = await client.chat.completions.create({
    ...
    messages: [
      { role: "system", content: buildSummarizerPrompt(FOCUS, USER_QUERY, DOC_ID) },
  ...
  console.log(response.choices[0]?.message?.content ?? "[No summary]");
